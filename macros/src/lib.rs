...
use quote::quote;
use syn::{punctuated::Punctuated, LitInt, Token, parse_macro_input, parse::Parse, Ident, token::Paren, parenthesized};
...
pub fn abilities(tokens : TokenStream) -> TokenStream {
    ...
    let iter = idents
        .into_iter()
        .map(|ident| {
            let id = format!("5E::{}",ident.to_string().to_uppercase());
            quote! {
                ...
                impl Saveable  for #ident {}
                impl Checkable for #ident {}
            }
        });
    ...
    let iter = abilities.into_iter()
        .flat_map(|ab| {
            ...
            ab.into_iter()
                .map(move |ref sk| {
                    let ab = &ab_ident;
                    let id = format!("5E::{}", sk.to_string().to_uppercase());
